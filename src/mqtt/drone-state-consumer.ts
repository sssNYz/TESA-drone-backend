...
mqttClient.subscribe([`${ARMY_PREFIX}#`], (err) => {
  if (err) console.error("❌ MQTT subscribe error (army/#)", err);
  else console.log(`✅ MQTT subscribed to ${ARMY_PREFIX}#`);
});

mqttClient.on("message", async (topic, buf) => {
  if (topic !== STATE_TOPIC) return; // only exact state topic for now
