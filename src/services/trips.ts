...
export type Waypoint = { lat: number; lon: number; alt_m?: number };
...
export type TripDetail = {
  ...
  droneId: string;
  waypoints: Waypoint[];
  actualPath: { ts: string; lat: number; lon: number }[];
  startsAt: string;
  estimatedEndAt: string;
  estimatedSeconds: number;
};
...
function normalizeWaypoints(waypoints: Waypoint[]): Waypoint[] {
...
function computePlannedSeconds(
  waypoints: Waypoint[],
  speedMS: number,
  currentStart?: { lat: number; lon: number } | null,
): { seconds: number; from: { lat: number; lon: number }; to: { lat: number; lon: number } } {
...
export async function createTrip(
  droneId: string,
  waypoints: Waypoint[],
  options?: { speedMS?: number; useCurrentAsStart?: boolean },
): Promise<TripDetail> {
  if (!droneId) throw new Error("droneId required");
  if (!Array.isArray(waypoints) || waypoints.length === 0) throw new Error("waypoints must be non-empty");
  ...
  return {
    ...
    waypoints: created.waypoints as any,
...
export async function listTrips(): Promise<TripSummary[]> {
  ...
  return rows.map((t) => ({
    ...
    waypointCount: Array.isArray(t.waypoints as any) ? (t.waypoints as any).length : 0,
  }));
}

export async function getTripDetail(id: string | number): Promise<TripDetail | null> {
